...
class Game(db.Model):
    ...
    category_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("game_categories.id")
    )
    ...
    type: Mapped[GameTypeEnum] = mapped_column(
        Enum(GameTypeEnum), nullable=False, index=True
    )

    # Flexible configuration
    config: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict)
    scoring_type: Mapped[GameScoringTypeEnum] = mapped_column(
        Enum(GameScoringTypeEnum), nullable=False, index=True
    )
    ...
    # Relationship
    category: Mapped[GameCategory] = relationship(
        "GameCategory", back_populates="games"
    )
    ...
    user_stats: Mapped[list[UserGameStats]] = relationship(
        "UserGameStats",
        back_populates="game",
        lazy="dynamic",
        cascade="all, delete-orphan",
