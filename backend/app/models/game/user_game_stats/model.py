...
from sqlalchemy import JSON, UUID, DateTime, Float, ForeignKey, Integer, UniqueConstraint
...
class UserGameStats(db.Model):
    ...
    last_played_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
    ...
    __table_args__ = (
        UniqueConstraint(
            'user_id', 'game_id', name='uq_user_game_stats'
        )
    )
