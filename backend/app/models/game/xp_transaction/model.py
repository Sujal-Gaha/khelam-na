...
class XPTransaction(db.Model):
    ...
    session_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('game_sessions.id'), nullable=True)

    xp_amount: Mapped[int] = mapped_column(Integer, nullable=False)
    type: Mapped[XPTransactionTypeEnum] = mapped_column(Enum(XPTransactionTypeEnum), nullable=False)
